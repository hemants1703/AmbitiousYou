...
export function NoteDetailDialog(props: NoteDetailDialogProps) {
  return (
    <Dialog open={props.note !== null} onOpenChange={props.onOpenChange}>
      {props.note ? (
        <NoteDetailContent key={`${props.note.id}:${props.note.updatedAt}`} note={props.note} ambitionName={props.ambitionName} onOpenChange={props.onOpenChange} />
      ) : null}
    </Dialog>
  );
}

function NoteDetailContent(props: { note: Note; ambitionName: string; onOpenChange: (open: boolean) => void }) {
  const notes = useAmbitionNotes();
  const [mode, setMode] = useState<DialogMode>("read");
  const [editingText, setEditingText] = useState(props.note.note);
