...
import { CalendarDaysIcon, CheckIcon, CircleCheckIcon, FlagIcon, RocketIcon, SparklesIcon, TargetIcon } from "lucide-react";
  ...
  return (
    <section ref={containerRef} className="w-full py-16 md:py-24">
      <div className=" px-4 md:px-6 max-w-5xl mx-auto">
        {/* Section Header */}
        <MotionWrapper initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-center mb-12">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 mb-4 text-sm font-medium bg-(--persona-color)/10 rounded-full" style={{ "--persona-color": props.persona.color } as React.CSSProperties}>
            <SparklesIcon className="h-4 w-4" style={{ color: props.persona.color }} />
            <span>Your Ambition in Action</span>
          </div>
          <h2 className="text-3xl md:text-4xl font-bold tracking-tight mb-3">Experience the journey</h2>
          <p className="text-muted-foreground max-w-xl mx-auto">This is how {props.persona.name.toLowerCase()} use AmbitiousYou. Try checking off items to see your progress update in real-time.</p>
        </MotionWrapper>
        ...
        <div className="grid lg:grid-cols-5 gap-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="flex items-start gap-3">
                    <div className="h-9 w-9 rounded-lg bg-purple-500/10 flex items-center justify-center shrink-0">
                      <FlagIcon className="h-4 w-4 text-purple-500" />
                    </div>
                    <div>
          ...
          <MotionWrapper initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.2 }} className="lg:col-span-3">
            <Card.Card className="h-full">
              <Card.CardHeader className="pb-3">
                ...
                <Card.CardDescription>Try clicking the checkboxes to see progress update</Card.CardDescription>
              </Card.CardHeader>
                      ...
                      <MotionWrapper key={item.id} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.05 }}>
                        <div
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            <span className="text-xs text-muted-foreground">{formatDate(itemDate, false)}</span>
        ...
        {/* Continue CTA */}
        <MotionWrapper initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.4 }} className="text-center mt-12">
