...
2. Set **Environment Variables** in the Vercel dashboard (Production / Preview): `DATABASE_URL`, `APP_BASE_URL`, optional `AZURE_CONNECTION_STRING`, VAPID keys, **`CRON_SECRET`** (required — daily ambition reminders via Vercel Cron in [`vercel.json`](vercel.json); see [`docs/notifications-cron.md`](docs/notifications-cron.md)).
